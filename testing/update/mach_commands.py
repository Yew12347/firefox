...
def get_binary_path(tempdir, **kwargs) -> str:
    ...
    if kwargs.get("source_versions_back"):
        ...
        source_version = (
            kwargs.get("source_version") or source_versions[DEFAULT_SOURCE_VERSION_POSITION]
        )
    ...
    if VERSION_INFO_FILENAME:
        ...
        print(f"Region: {source_locale}\nSource Version: {source_version}\nPlatform: {os_edition}")

    executable_url = (
        rf"{FX_DOWNLOAD_DIR_URL}{source_version}/{platform}/{source_locale}/{executable_name}"
    )
...
def build(command_context, binary_path, **kwargs):
    ...
    try:
        ...
        return run_tests(topsrcdir=command_context.topsrcdir, tempdir=tempdir_name, **kwargs)
